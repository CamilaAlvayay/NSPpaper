...
Figure~\ref{fig:trends_sale} provides a visual comparison of log sale prices over time across the three groups: never-treated block groups, the NSP1 cohort (treated in 2009), and the NSP3 cohort (treated in 2013). Prior to 2009, the log sale price trends are approximately parallel across all three groups, supporting the conditional parallel trends assumption underlying our identification strategy. After the respective treatment dates, the trajectories diverge modestly, with NSP1 areas showing somewhat lower prices---a pattern consistent with the selection mechanism that directed NSP funds to the most distressed neighborhoods. The NSP3 cohort, which was treated later (2013), shows a trajectory closer to the control group in the pre-2013 period, providing additional identifying variation for the staggered design.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{pretrend_lprice_staggered.png}
\caption{Parallel Trends: ln(Sale Price) by Treatment Cohort}
...
Figure~\ref{fig:trends_fore} presents the analogous comparison for foreclosure rates. Pre-treatment trends are closely aligned across all three groups, with foreclosure rates rising sharply during the crisis period (2007--2009) and then declining gradually. The close tracking of the three groups in the pre-treatment period provides visual support for the parallel trends assumption, particularly for the foreclosure rate outcome where the formal pre-trend test yields a $p$-value of $0.066$.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{pretrend_forerate_staggered.png}
\caption{Parallel Trends: Foreclosure Rate by Treatment Cohort}
...
\midrule
Sale Price (\$) & 23,352 & 25,768 & 19,798 & 16,812 & $0.017^{**}$ \\
ln(Sale Price) & 9.62 & 1.00 & 9.55 & 0.93 & $0.263$ \\
Foreclosure Rate & 0.09 & 0.13 & 0.12 & 0.16 & $0.001^{***}$ \\
Number of Sales & 12.85 & 10.24 & 16.17 & 13.16 & $< 0.001^{***}$ \\
\addlinespace
...
\begin{tablenotes}[flushleft]
\footnotesize
\item \textit{Source:} Authors' calculations. \textit{Note:} $p$-values from two-sample $t$-tests of equality of means. HUD formula variables are time-invariant. $^{***} p < 0.01$, $^{**} p < 0.05$, $^{*} p < 0.10$.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
Table~\ref{tab:main_results} presents our main results from the \citet{callaway2021} estimator, which exploits the staggered timing of NSP1 (2009) and NSP3 (2013) to identify treatment effects using not-yet-treated and never-treated block groups as the comparison group. All specifications include the HUD formula variables as controls.

Panel~A shows results for log sale prices. The overall ATT across both cohorts is $-0.033$ ($\text{SE} = 0.054$, $p = 0.537$), indicating no statistically significant effect of the NSP on housing prices. The cohort-specific estimates reveal heterogeneity: the NSP1 cohort shows a small negative effect ($-0.038$, $p = 0.505$), while the NSP3 cohort shows a small positive but insignificant effect ($+0.033$, $p = 0.698$). The formal pre-trend test fails to reject the null of parallel pre-treatment trends at the 10\% level ($\chi^2(8) = 12.74$, $p = 0.121$), supporting our identification strategy.

Panel~B shows results for the foreclosure rate. The overall ATT is $+0.080$ ($\text{SE} = 0.052$, $p = 0.121$), suggesting a modest but statistically insignificant increase in foreclosure rates. The cohort-specific estimates again show heterogeneity: the NSP1 cohort has a positive effect ($+0.089$, $p = 0.102$), while the NSP3 cohort shows a negative (stabilizing) effect ($-0.048$, $p = 0.545$). The pre-trend test yields $\chi^2(8) = 14.65$ ($p = 0.066$), marginally passing at the 5\% level.

The contrast between cohorts is informative: NSP1 was implemented at the height of the foreclosure crisis (2009), when neighborhood conditions were deteriorating rapidly, while NSP3 arrived later (2013) when housing markets had begun to stabilize. The null-to-positive NSP3 price effect is consistent with the program achieving modest stabilization in a less turbulent environment.

\begin{table}[htbp]
\centering
\caption{Main Results: Callaway and Sant'Anna Staggered DiD Estimates}
\label{tab:main_results}
\begin{threeparttable}
\begin{tabular}{lccc}
\toprule
& Overall ATT & NSP1 Cohort (2009) & NSP3 Cohort (2013) \\
\midrule
\multicolumn{4}{l}{\textbf{Panel A: ln(Sale Price)}} \\
\addlinespace
ATT & $-0.033$ & $-0.038$ & $+0.033$ \\
& $(0.054)$ & $(0.057)$ & $(0.085)$ \\
$p$-value & $0.537$ & $0.505$ & $0.698$ \\
\addlinespace
Pre-trend $\chi^2$ & \multicolumn{3}{c}{$12.74$ \quad ($p = 0.121$)} \\
Observations & \multicolumn{3}{c}{$14{,}924$} \\
\addlinespace
\multicolumn{4}{l}{\textbf{Panel B: Foreclosure Rate}} \\
\addlinespace
ATT & $+0.080$ & $+0.089^{*}$ & $-0.048$ \\
& $(0.052)$ & $(0.055)$ & $(0.080)$ \\
$p$-value & $0.121$ & $0.102$ & $0.545$ \\
\addlinespace
Pre-trend $\chi^2$ & \multicolumn{3}{c}{$14.65^{*}$ \quad ($p = 0.066$)} \\
Observations & \multicolumn{3}{c}{$14{,}924$} \\
\addlinespace
\midrule
Control group & \multicolumn{3}{c}{Not-yet treated + never treated} \\
HUD formula controls & \multicolumn{3}{c}{Yes} \\
Block groups & \multicolumn{3}{c}{$1{,}066$ (399 control, 599 NSP1, 68 NSP3)} \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\footnotesize
\item \textit{Source:} Authors' calculations. \textit{Note:} Estimated using the \citet{callaway2021} estimator with doubly robust (IPW) inference and staggered treatment timing ($g = 2009$ for NSP1, $g = 2013$ for NSP3). HUD formula controls include predicted foreclosure rate, subprime loan rate, and residential vacancy rate. Standard errors in parentheses. Pre-trend test: $H_0$: all pre-treatment event-time coefficients equal zero. $^{***} p < 0.01$, $^{**} p < 0.05$, $^{*} p < 0.10$.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
Figure~\ref{fig:event_sale} presents the event study results for log sale prices from the staggered Callaway and Sant'Anna estimator with HUD controls. The event-time coefficients aggregate across both treatment cohorts (NSP1 and NSP3), with the not-yet-treated and never-treated groups serving as the comparison. The pre-treatment coefficients (Tm6 through Tm1) are centered around zero, with only Tm2 marginally significant---consistent with the formal pre-trend test ($p = 0.121$). Post-treatment, the coefficients oscillate around zero without any clear trend, and no individual coefficient exceeds the 5\% significance threshold, consistent with the absence of a systematic price effect.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{cs_staggered_sale_event.png}
\caption{Event Study: ln(Sale Price) --- Staggered C\&S Design}
\label{fig:event_sale}
\begin{minipage}{\textwidth}
\footnotesize
\textit{Source:} Authors' calculations. \textit{Note:} Aggregated event study coefficients from the \citet{callaway2021} estimator exploiting staggered treatment timing (NSP1 in 2009, NSP3 in 2013). HUD formula controls included. The vertical dashed line marks event time zero (treatment onset). 95\% confidence intervals shown.
\end{minipage}
\end{figure}

Figure~\ref{fig:event_fore} presents the event study for the foreclosure rate. The pre-treatment coefficients are close to zero, consistent with the pre-trend test ($p = 0.066$). Post-treatment, the coefficients show a distinctive pattern: the effect is initially small (Tp0--Tp2), then spikes sharply at Tp3 and Tp5---corresponding to 3 and 5 years after treatment---before declining. The peak effect at Tp5 ($+0.325$, $p < 0.05$) suggests that treated areas experienced temporarily higher foreclosure rates in the medium run, likely reflecting the continued unwinding of distressed mortgages in the neighborhoods most severely affected by the crisis. By Tp8--Tp10, the effects converge toward zero, indicating long-run stabilization.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{cs_staggered_fore_event.png}
\caption{Event Study: Foreclosure Rate --- Staggered C\&S Design}
